...
# set requested geo information datasets @todo: can be get from a configuration file
# for each item, the first value in the value list would be the dataset "ProductAndVersion"
# the rest would be the layers that you want to get. (Some dataset contains multiple layers.)
geodata_name_dict = {"DEM":['SRTMGL1_NC.003',],"Land Cover" : ["MCD12Q1",]} 
## only these two types are supported,only tiff file will be auto converted to the output coordinate system
output_format_dict     = {"DEM":'geotiff',"Land Cover": 'netcdf4'} 
center_lat = -43.247
center_lon = 171.608
area_radius = 15000  # units=metre
default_proj = "EPSG:4326" # used to know what coords center_lat and center_lon is in
# output_proj = "EPSG:2193"
start_date_dict = {"DEM":'',"Land Cover":"2020-10-01"} ## YYYY-MM-DD
end_date_dict = {"DEM":'',"Land Cover":"2020-10-30"} ## YYYY-MM-DD
task_name_prefix = "test_chc"
output_dir = "/home/jzh/analysis/test_chc" 
default_buffer_ratio = 1.1 # used to multiply area_radius avoid areas becoming smaller than required after reproject
...
def check_config_dataset(geodata_name_dict,product_response):
...
def get_req_bbox(center_lon,cente_lat,area_radiu,default_proj,default_buffer_ratio):
    # get the area bbox based on center_lon, center_lat and area_radius
...
def submit_tasks(geodata_name_dict,start_date_dict,end_date_dict,output_format_dict,task_name_prefix,req_proj_name,crop_area_jason_input,api,head):
    ...
    if len(task_response_list) == 0:
        sys.exit("No tasks to submit. Please check your configurations.")
...
if __name__ == "__main__":
    ...
    check_config_dataset(geodata_name_dict,product_response)
    ...
    crop_area_jason_input = get_req_bbox(center_lon,center_lat,area_radius,default_proj,default_buffer_ratio)
    ...
    task_response_list=submit_tasks(geodata_name_dict,start_date_dict,end_date_dict,output_format_dict,\
                                    task_name_prefix,req_proj_name,crop_area_jason_input,api,head)

    check_tasks_status(task_response_list,api,head,60.0)

    data_download(task_response_list,output_dir,api,head)